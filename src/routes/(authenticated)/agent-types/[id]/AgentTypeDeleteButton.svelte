<script lang="ts">
	...
	export let product: _AgentType;
	const isDialogOpen = atom(false);
	const appState = getAppState();
	const toastState = getToastState();

	async function handleDeleteAgentType() {
		try {
			isDialogOpen.set(false);
			appState.loading.set(true);
			await AgentTypeRepository.delete(product.id);
...
<Dialog isOpen={isDialogOpen}>
	<div class="w-96 flex flex-col gap-3">
		...
		<div class="flex gap-2">
			<Button label="Yes" onClick={handleDeleteAgentType} />
			<Button
				label="No"
				class="button-red"
				onClick={() => {
